    ...
    async def async_step_user(self, user_input=None):
        ...
        if user_input is not None:
            try:
                ...
                _LOGGER.exception("Unexpected error validating Swift Command login")
                errors["base"] = "unknown"
            else:
    ...
    async def async_step_reauth_confirm(self, user_input=None):
        ...
        if user_input is not None:
            try:
                ...
                _LOGGER.exception("Unexpected error during Swift Command reauth")
                errors["base"] = "unknown"
            else:
        ...
        schema = vol.Schema(
            {
                ...
                vol.Required(
                    CONF_CAN_BUS_TIMEOUT,
                    default=options.get(CONF_CAN_BUS_TIMEOUT, DEFAULT_CAN_BUS_TIMEOUT),
                ): selector.NumberSelector(
                    selector.NumberSelectorConfig(
                        min=5,
                        max=60,
                ...
                vol.Optional(
                    CONF_CAN_SECTIONS,
                    default=options.get(CONF_CAN_SECTIONS, DEFAULT_CAN_SECTIONS),
                ): selector.SelectSelector(
                    selector.SelectSelectorConfig(
                        options=DEFAULT_CAN_SECTIONS,
                        multiple=True,
